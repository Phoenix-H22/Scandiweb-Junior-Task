...
function Navbar() {
  ...
  useEffect(() => {
    ...
    const setInitialCategory = () => {
      ...
      const category = data.categories.find((cat) => cat.name.toLowerCase() === path.toLowerCase());
  ...
  if (loading)
    return (
      <header className="relative h-[80px] flex items-center justify-between">
        <nav>
          <ul className="flex items-center">
            <li className="relative text-[18px] uppercase px-2 sm:px-4">
              <Link to="/all" data-testid="category-link">
                ALL
              </Link>
            </li>
          </ul>
  ...
  const isActiveCategory = (category: Category) => {
    ...
    return currentPath === categoryPath || (categoryPath === "/all" && currentPath === "/");
  };

  return (
    <header className="sticky top-0 bg-white z-10 h-[80px] flex items-center justify-between">
      ...
      <Link to="/">
        <img src="/icons/ecommerce-logo.svg" alt="ecommerce-logo" />
      </Link>
